...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
	 ...
	 * 得到自己定义的，并且没有交给spring管理的bean，也就是没有加上 @Component
	 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 配置bean工厂，里面实现了BeanDefinition的装载。 获得 beanfactory
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 配置bean工厂的上下文信息，如类装载器等
			// 为bean工厂添加属性，向bean工厂中添加BeanPostProcessor
			...
			try {
				...
				// might not ever need metadata for singleton beans anymore... 重置Spring核心中的公共自省缓存，因为我们可能再也不需要单例bean的元数据了……
				resetCommonCaches();
			}
		}
	}
	 ...
	 *
	 * 实例化并调用所有已注册的BeanFactoryPostProcessor bean，如果显示的指定顺序，则要按照指定的顺序。
	 * 必须在单例实例化之前调用。
	 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		// Initialize conversion service for this context.  为context初始化转换服务
